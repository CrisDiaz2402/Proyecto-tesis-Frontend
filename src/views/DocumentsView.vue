  ...
  <div class="p-4 sm:p-6 lg:p-8">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
      <div>
        ...
        <p class="text-gray-500 text-sm mt-1">Base de conocimiento del asistente virtual</p>
      </div>
      ...
      <div class="flex flex-col items-start sm:items-end">
        ...
        <span v-if="documentos.length >= MAX_DOCUMENTOS" class="text-xs text-red-500 mt-2">
    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden mb-8 shadow-sm">
      <div class="px-4 sm:px-6 py-4 border-b border-gray-200 flex flex-wrap justify-between items-center gap-3">
      ...
      <div class="p-4 sm:p-6 flex flex-wrap gap-3 sm:gap-4 items-center bg-gray-50">

        <button
          @click="pedirConfirmacion('cache')"
          :disabled="procesandoGlobal"
          class="flex items-center gap-2 px-4 py-2 bg-yellow-50 hover:bg-yellow-100 text-yellow-700 border border-yellow-300 rounded-lg text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          <Icon icon="mdi:broom" />
          Limpiar Caché
        </button>
        ...
        <button
          ...
          :disabled="procesandoGlobal"
          class="flex items-center gap-2 px-4 py-2 bg-orange-50 hover:bg-orange-100 text-orange-700 border border-orange-300 rounded-lg text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
        ...
        <button
          ...
          :disabled="procesandoGlobal"
          class="flex items-center gap-2 px-4 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-300 rounded-lg text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
      <div class="px-6 py-4 border-b border-gray-200 flex items-center gap-2">
        <Icon icon="mdi:file-document-multiple-outline" class="text-blue-500 text-lg" />
        <h2 class="text-sm font-semibold text-gray-800">Base de Conocimiento</h2>
      ...
      <div v-else-if="documentos.length === 0" class="p-12 text-center">
        <Icon icon="mdi:file-hidden" class="text-6xl text-gray-300 mx-auto mb-4" />
        <p class="text-gray-500">No hay documentos en el sistema.</p>
        <p class="text-gray-400 text-sm mt-1">Sube archivos PDF, Word, TXT o Markdown para empezar.</p>
      </div>
      ...
      <table class="w-full text-left text-sm text-gray-700">
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="doc in documentos" :key="doc.id" class="hover:bg-gray-50 transition-colors group">
            <td class="px-4 sm:px-6 py-4 font-medium text-gray-800 flex items-center gap-3">
            ...
            <td class="px-4 sm:px-6 py-4">
              <div class="flex flex-col gap-1.5">
                <span v-if="doc.procesado_local" class="px-2 py-1 bg-blue-50 text-blue-700 text-[10px] uppercase font-bold rounded border border-blue-200 w-fit flex items-center gap-1">
                  🖥️ {{ doc.estado_local }}
                </span>
                <span v-else class="px-2 py-1 bg-gray-100 text-gray-500 text-[10px] uppercase font-bold rounded border border-gray-200 w-fit flex items-center gap-1">
                  🖥️ No en Local
                </span>
              </div>
            </td>
            <td class="px-4 sm:px-6 py-4 text-right">
              <div class="flex items-center justify-end gap-3">
                <button v-if="doc.procesado_local" @click="descargar(doc, 'local')" class="text-blue-500 hover:text-blue-600 transition-colors" title="Descargar de Local">
                  <Icon icon="mdi:monitor-arrow-down" class="text-xl" />
                </button>

                <div class="w-px h-5 bg-gray-200 mx-1"></div>

                <button @click="abrirModalEliminarDoc(doc)" class="text-red-500 hover:text-red-600 transition-colors" title="Eliminar documento">
    ...
    <div v-if="modalSubirAbierto" class="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-2xl w-full max-w-md p-4 sm:p-6 border border-gray-200 shadow-xl relative animate-pop-in">
        <button @click="cerrarModalSubir" class="absolute top-4 right-4 text-gray-400 hover:text-gray-700 transition-colors">
          <Icon icon="mdi:close" class="text-xl" />
        </button>

        <h3 class="text-xl font-bold text-gray-800 mb-1">Subir Documento</h3>
        <p class="text-gray-500 text-sm mb-5">Añade información a la base de conocimiento.</p>

        <div
          class="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center bg-gray-50 hover:bg-gray-100 hover:border-gray-400 transition-colors cursor-pointer mb-5"
        ...
        >
          ...
          <div v-if="!archivoSeleccionado">
            ...
            <p class="text-gray-800 font-medium">Haz clic para buscar</p>
          ...
          <div v-else class="flex flex-col items-center gap-2">
            ...
            <p class="text-gray-800 font-medium truncate w-full px-4">{{ archivoSeleccionado.name }}</p>
        ...
        <div v-if="mensajeModalSubir" :class="[
          ...
          mensajeModalSubir.tipo === 'error' ? 'bg-red-50 text-red-600 border border-red-200' : 'bg-green-50 text-green-600 border border-green-200'
        ]">
        ...
        <div class="flex justify-end gap-3">
          <button @click="cerrarModalSubir" class="px-4 py-2 text-sm font-medium text-gray-500 hover:text-gray-800 transition-colors">Cancelar</button>
          <button
    ...
    <div v-if="modalEliminarDoc.show" class="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div class="bg-white border border-red-200 rounded-2xl p-4 sm:p-6 w-full max-w-sm shadow-xl relative animate-pop-in">
        <h3 class="text-lg font-bold text-gray-800 mb-2 flex items-center gap-2">
          <Icon icon="mdi:alert-circle-outline" class="text-red-500 text-xl" /> Eliminar Documento
        </h3>

        <p class="text-gray-500 text-sm mb-4">
          Confirma que deseas eliminar <strong class="text-gray-800">{{ modalEliminarDoc.doc?.nombre_archivo }}</strong> del sistema.
        </p>

        <div class="bg-gray-50 border border-gray-200 rounded-lg p-3 mb-4 text-xs text-gray-500 space-y-1">
          <p class="text-gray-700 font-semibold mb-1">⚠️ Al eliminar el documento se borra:</p>
        ...
        <div class="space-y-3 mb-6">
          <label
            ...
            class="flex items-center gap-3 p-3 bg-white border border-gray-200 rounded-lg cursor-pointer hover:border-red-400 transition-colors"
            :class="{ 'border-red-500 bg-red-900/10': eliminarOpciones.local }"
          >
            <input type="checkbox" v-model="eliminarOpciones.local" class="w-4 h-4 text-red-600 bg-white border-gray-300 rounded focus:ring-red-600 focus:ring-2">
        ...
        <div class="flex gap-3">
          <button
            ...
            class="flex-1 py-2 bg-gray-100 hover:bg-gray-200 text-gray-600 text-sm font-medium rounded-lg transition-colors disabled:opacity-50"
          >Cancelar</button>
          <button
    ...
    <div v-if="modalFormatear.show" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[100] px-4" @click.self="cerrarModalFormatear">
      <div class="bg-white border border-red-200 rounded-2xl p-6 w-full max-w-md shadow-xl animate-pop-in">
        <div class="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 class="text-lg font-bold text-gray-800">Formatear Sistema</h3>
            <p class="text-red-500 text-xs font-medium">Acción irreversible</p>
          </div>
          <button @click="cerrarModalFormatear" :disabled="modalFormatear.loading" class="ml-auto text-gray-400 hover:text-gray-700">
            <Icon icon="mdi:close" class="text-xl" />
          </button>
        </div>
        <div class="bg-red-50 border border-red-200 rounded-xl p-4 mb-5">
          <p class="text-red-700 text-xs font-semibold uppercase tracking-wider mb-2">Se eliminará permanentemente:</p>
          <ul class="space-y-1.5">
            <li class="flex items-center gap-2 text-gray-700 text-sm"><Icon icon="mdi:circle-small" class="text-red-500" /> Todos los documentos</li>
            <li class="flex items-center gap-2 text-gray-700 text-sm"><Icon icon="mdi:circle-small" class="text-red-500" /> Vectores de Qdrant (Local)</li>
            <li class="flex items-center gap-2 text-gray-700 text-sm"><Icon icon="mdi:circle-small" class="text-red-500" /> Caché semántico completo (local:local y local:cloud)</li>
          </ul>
        </div>
        <div class="mb-5">
          <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Confirma tu contraseña</label>
          <div class="relative">
            <Icon icon="mdi:lock-alert-outline" class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-lg" />
            <input
              ...
              class="w-full pl-10 pr-10 py-2.5 bg-gray-50 border border-gray-300 text-gray-800 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500"
            />
            <button type="button" @click="modalFormatear.verPassword = !modalFormatear.verPassword" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
              <Icon :icon="modalFormatear.verPassword ? 'mdi:eye-off-outline' : 'mdi:eye-outline'" class="text-lg" />
            </button>
          </div>
        </div>
        <div class="flex gap-3">
          <button @click="cerrarModalFormatear" :disabled="modalFormatear.loading" class="flex-1 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-600 text-sm font-semibold rounded-lg">Cancelar</button>
          <button
...
function detalleCacheMotor(m: MotorScope): string {
  return {
    local: 'cache_ll (local:local)', // DISABLED: y cache_lc (local:cloud)
  }[m] ?? m
}
  ...
  const config = {
    ...
    vectores: {
      ...
      mensaje:
        ...
        `El estado de los documentos en BD se actualizará a "No subido".`,
